...
<template>
  <main class="container">
    ...
    <p class="new-year">🎉 Wishing You a Happy New Year Ahead! 🎉</p>
    <div class="confetti">
      <div 
        v-for="i in 50" 
        :key="i" 
...
// Reactive Myanmar time
const myanmarTime = ref('');

function updateMyanmarTime() {
  const now = new Date();
  const utc = now.getTime() + now.getTimezoneOffset() * 60000;
  const myanmarOffset = 6.5 * 60 * 60000; // Myanmar UTC+6:30
  const myanmarDate = new Date(utc + myanmarOffset);
  myanmarTime.value = myanmarDate.toLocaleTimeString('en-GB', { hour12: false });
}
...
@keyframes sparkle {
  0% { text-shadow: 0 0 5px #ffd700, 0 0 10px #ffea00; }
  100% { text-shadow: 0 0 20px #ffd700, 0 0 40px #ffea00; }
}
